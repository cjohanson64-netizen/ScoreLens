...
export function evaluateBindExpr(
  ...
  switch (expr.type) {
    ...
    case "FirstExpr":
      return evaluateBindFirstExpr(expr, bindings, actions, graph);
...
function evaluateBindFirstExpr(
  expr: FirstExprNode,
  bindings: RuntimeBindings,
  actions: ActionRegistry,
  graph: Graph | null,
): BindValue {
  const value = evaluateBindExpr(expr.expression, bindings, actions, graph);
  if (Array.isArray(value)) {
