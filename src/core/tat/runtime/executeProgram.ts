...
function evaluatePruneExpr(
  ...
  switch (expr.type) {
    ...
    case "ComparisonExpr": {
      ...
      return applyPruneComparisonOperator(expr.operator, left, right);
    }
...
function applyPruneComparisonOperator(
  ...
  switch (operator) {
    case "==":
      return comparePruneValue(left, right);
    case "!=":
      return !comparePruneValue(left, right);
    case "<":
      return comparePruneOrdered(left, right, (a, b) => a < b);
    case "<=":
      return comparePruneOrdered(left, right, (a, b) => a <= b);
    case ">":
      return comparePruneOrdered(left, right, (a, b) => a > b);
    case ">=":
      return comparePruneOrdered(left, right, (a, b) => a >= b);
    default:
      throw new Error(`Malformed @where predicate syntax: unsupported comparison operator "${operator}"`);
  }
}

function comparePruneOrdered(
  left: GraphValue | undefined,
  right: GraphValue | undefined,
