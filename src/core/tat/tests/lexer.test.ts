...
test("tokenizes ordered comparison operators in @where", () => {
  const source = `@where(node.meta.measureNumber >= 2 && node.meta.measureNumber <= 4 && node.meta.voice > "0" && node.meta.voice < "9")`;
...
test("tokenizes @first around a @where expression", () => {
  const source = `@first(@where(node.meta.measureDefaultTargetRank == 0))`;
