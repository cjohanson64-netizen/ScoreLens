...
test("parses ordered comparisons in @where query", () => {
  const ast = parseSource(`
@where(node.meta.measureNumber >= 2 && node.meta.measureNumber <= 4)
`);

  const stmt = ast.body[0];
  assert.equal(stmt.type, "QueryStatement");
  assert.equal(stmt.expr.type, "WhereExpr");
  assert.equal(stmt.expr.expression.type, "BinaryBooleanExpr");
});

test("parses @first value expressions", () => {
  const ast = parseSource(`
@bind.state.node(selectedMeasureDefaultTarget := @first(@where(node.meta.measureDefaultTargetRank == 0)))
`);

  const stmt = ast.body[0];
  assert.equal(stmt.type, "BindStatement");
  assert.equal(stmt.expression.type, "FirstExpr");
  assert.equal(stmt.expression.expression.type, "WhereExpr");
});
