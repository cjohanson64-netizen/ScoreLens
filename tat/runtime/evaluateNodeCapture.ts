...
export function evaluateValueExpr(
  ...
  switch (expr.type) {
    ...
    case "FirstExpr":
      return evaluateFirstExpr(expr, bindings, actions);
...
function evaluateFirstExpr(
  expr: FirstExprNode,
  bindings: RuntimeBindings,
  actions: ActionRegistry,
): GraphValue {
  const value = evaluateValueExpr(expr.expression, bindings, actions);
...
function printTraversalValue(expr: ValueExprNode): string {
  switch (expr.type) {
    ...
    case "FirstExpr":
      return `@first(${printTraversalValue(expr.expression)})`;
    case "ObjectLiteral":
